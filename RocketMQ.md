...
6) 向broker注册消费者。
7) 启动客户端实例 服务状态更新为：RUNNING[]()### 2.3.2 其它消息存储、消息查询、通信机制-通信架构图、负载均衡-Producer的负载均衡、事务消息（半消息事务）、事务消息-broker2PC设计、事务消息-Op消息等。--限于篇幅，就不都举例了，详情见[RocketMQ详解（四）核心设计原理（源码级剖析必读）](https://www.cnblogs.com/dennyzhangdd/p/15035116.html "发布于 2021-07-20 16:05")。[回到顶部(go to top)](#_labelTop)[]()# 三、总结本文主要想表达的是，作为一名架构师，要能成体系的去研究技术。兼顾广度和深度。核心原理是架构师的技术域的核心，只有掌握了核心原理，各种问题迎刃而解。剩下的无非是一些基于原理上的应用、调优、二次开发。 
