...
def agent\_run(user\_goal: str):
    \# 第一步：规划模块上场，把目标拆成步骤列表
    plan = llm.plan(user\_goal)
    ...
    for step in plan:
        ...
        action = llm.decide(
            ...
            long\_term=vector\_db.search(step)   \# 从长期记忆里捞出相关历史
        )
        if action.type == "tool\_call":
            \# 第三步：工具系统负责真正执行
            result = tools.execute(action.tool\_name, action.args)
            memory.append({"step": step, "result": result})  \# 执行结果存入短期记忆
        elif action.type == "final\_answer":
...
[![](https://seven97-blog.oss-cn-hangzhou.aliyuncs.com/imgs/202603102211214.png)](https://seven97-blog.oss-cn-hangzhou.aliyuncs.com/imgs/202603102211214.png)
...
[![](https://seven97-blog.oss-cn-hangzhou.aliyuncs.com/imgs/202603180859126.png)](https://seven97-blog.oss-cn-hangzhou.aliyuncs.com/imgs/202603180859126.png)
...
[![](https://seven97-blog.oss-cn-hangzhou.aliyuncs.com/imgs/202508031634674.png)](https://seven97-blog.oss-cn-hangzhou.aliyuncs.com/imgs/202508031634674.png)
...
def plan\_and\_execute(question: str, tools: dict):
    ...
    for i, step in enumerate(steps):
        \# 每一步可以用 ReAct 模式来执行
        \# 但执行器知道自己在整体计划中的位置
        step\_result = react\_executor(
            task=step,
            tools=tools,
            context=f"整体计划共{len(steps)}步，当前是第{i+1}步",
            previous\_results=results  \# 把前面步骤的结果传进去
        )
        results.append(step\_result)
        
        \# 关键：执行完一步后，检查是否需要调整后续计划
        \# 这就是「动态重规划」机制
        if need\_replan(step, step\_result, steps\[i+1:\]):
            remaining\_steps = llm.generate(f"""
            原计划：{steps}
            已完成到第{i+1}步，结果：{results}
            剩余步骤是否需要调整？请输出更新后的剩余步骤。
            """)
            steps = steps\[:i+1\] + parse\_plan(remaining\_steps)
...
[![](https://seven97-blog.oss-cn-hangzhou.aliyuncs.com/imgs/202603102235511.png)](https://seven97-blog.oss-cn-hangzhou.aliyuncs.com/imgs/202603102235511.png)
...
-   [什么是大模型 Agent？它与传统的 AI 系统有什么不同？](#什么是大模型-agent它与传统的-ai-系统有什么不同)
-   [什么是 AI Agent？其核心思想是什么？](#什么是-ai-agent其核心思想是什么)
-   [LLM Agent 的基本架构有哪些组成部分？](#llm-agent-的基本架构有哪些组成部分)
-   [Agent智能体的工作过程是怎样的？](#agent智能体的工作过程是怎样的)
-   [什么是 Agent Loop？其工作流程是什么？](#什么是-agent-loop其工作流程是什么)
-   [Workflow，Agent，Tools 这三个的概念和区别介绍一下？](#workflowagenttools-这三个的概念和区别介绍一下)
-   [Workflow和Agent的区别是什么？](#workflow和agent的区别是什么)
-   [Agent 推理模式有哪些？](#agent-推理模式有哪些)
-   [如何赋予 LLM 规划能力？](#如何赋予-llm-规划能力)
-   [什么是 CoT 思维链？如何实现 CoT 思维链？](#什么是-cot-思维链如何实现-cot-思维链)
-   [什么是 CoT 思维链和 ReAct 模式？它们如何提高 AI 推理能力？](#什么是-cot-思维链和-react-模式它们如何提高-ai-推理能力)
-   [ReAct 是什么？说说它的原理](#react-是什么说说它的原理)
-   [什么是 ReAct？如何基于 ReAct 模式构建具备自主规划能力的 AI 智能体？](#什么是-react如何基于-react-模式构建具备自主规划能力的-ai-智能体)
-   [能否通过你的项目中实际的例子体现 ReAct 模式？](#能否通过你的项目中实际的例子体现-react-模式)
-   [ReAct 是怎么实现的？](#react-是怎么实现的)
-   [什么是 Plan-and-Execute 模式？](#什么是-plan-and-execute-模式)
-   [什么是 Reflection 模式？](#什么是-reflection-模式)
-   [讲讲 Agent 的反思机制？为什么要用反思？具体怎么实现？](#讲讲-agent-的反思机制为什么要用反思具体怎么实现)
-   [还了解其它Agent设计模式吗？](#还了解其它agent设计模式吗)
-   [ReAct、Plan-and-Execute、Reflection 三种范式有什么核心区别？实际项目中该如何选型？](#reactplan-and-executereflection-三种范式有什么核心区别实际项目中该如何选型)
-   [复杂任务怎么做的任务拆分？为什么要拆分？效果如何提升？](#复杂任务怎么做的任务拆分为什么要拆分效果如何提升)
-   [Context Engineering 包含哪些内容？](#context-engineering-包含哪些内容)
-   [Context Engineering 包含哪些核心技术？](#context-engineering-包含哪些核心技术)
-   [请你介绍一下 AI Agent 的记忆机制](#请你介绍一下-ai-agent-的记忆机制)
-   [在实际开发中应该如何设计Agent的记忆模块？](#在实际开发中应该如何设计agent的记忆模块)
-   [Context Window 管理：短期记忆不够大怎么办？Agent 记忆压缩通常有哪些方法？](#context-window-管理短期记忆不够大怎么办agent-记忆压缩通常有哪些方法)
-   [如何让 LLM Agent 具备长期记忆能力？](#如何让-llm-agent-具备长期记忆能力)
-   [Agent 的长短期记忆系统怎么做的？记忆是怎么存的？粒度是多少？怎么用的？](#agent-的长短期记忆系统怎么做的记忆是怎么存的粒度是多少怎么用的)
-   [什么是 Multi-Agent 系统？](#什么是-multi-agent-系统)
-   [说说 Single-Agent 和 Multi-Agent 的设计方案？](#说说-single-agent-和-multi-agent-的设计方案)
-   [在工程实践中，为什么有时候选择手搓Agent，而不是直接用成熟框架？](#在工程实践中为什么有时候选择手搓agent而不是直接用成熟框架)
-   [如何设计多 Agent 的协作与动态切换机制？](#如何设计多-agent-的协作与动态切换机制)
-   [什么是 OpenManus？它的实现原理是什么？](#什么是-openmanus它的实现原理是什么)
-   [Copilet 模式和 Agent 模式的区别是什么？](#copilet-模式和-agent-模式的区别是什么)
-   [Agent 死循环问题有遇到过吗?如何解决?](#agent-死循环问题有遇到过吗如何解决)
-   [什么是 Prompt Injection（提示词注入攻击）？](#什么是-prompt-injection提示词注入攻击)
-   [LLM Agent 在多模态任务中如何执行推理？](#llm-agent-在多模态任务中如何执行推理)

  

\_\_EOF\_\_

[![](https://images.cnblogs.com/cnblogs_com/blogs/829136/galleries/2416660/t_240818044824_750.png)](https://images.cnblogs.com/cnblogs_com/blogs/829136/galleries/2416660/t_240818044824_750.png)

-   **本文作者：** [Seven](https://www.cnblogs.com/sevencoding)
-   **本文链接：** [https://www.cnblogs.com/sevencoding/p/20007274](https://www.cnblogs.com/sevencoding/p/20007274)
-   **关于博主：** Seven的菜鸟成长之路
...
分类: [精选面试题](https://www.cnblogs.com/sevencoding/category/2453886.html)
...
[![](https://pic.cnblogs.com/face/3505237/20240818113531.png)](https://home.cnblogs.com/u/sevencoding/)

[程序员Seven](https://home.cnblogs.com/u/sevencoding/) [![](https://assets.cnblogs.com/vip.png "博客园VIP会员")](https://home.cnblogs.com/u/sevencoding/)  
[粉丝 - 183](https://home.cnblogs.com/u/sevencoding/followers/) [关注 - 0](https://home.cnblogs.com/u/sevencoding/followees/)  
[会员号：6086](https://cnblogs.vip/)
...
[«](https://www.cnblogs.com/sevencoding/p/19974719) 上一篇： [Tomcat Request请求处理过程：Connector](https://www.cnblogs.com/sevencoding/p/19974719 "发布于 2026-05-08 09:00")

posted @ 2026-05-11 09:00  [程序员Seven](https://www.cnblogs.com/sevencoding)  阅读(195)  评论(0)    [收藏](javascript:void\(0\))  [举报](https://report.cnblogs.com?targetLink=https%3A%2F%2Fwww.cnblogs.com%2Fsevencoding%2Fp%2F20007274&targetId=20007274&targetType=0)

var cb\_entryId = 20007274, cb\_entryCreatedDate = '2026-05-11 09:00', cb\_postType = 1, cb\_postTitle = '万字长文背诵最新Agent常见试题'; var allowComments = true, cb\_blogId = 829136, cb\_blogApp = 'sevencoding', cb\_blogUserGuid = '3a87eb84-a8ab-4b68-f697-08dcba07fc13'; mermaidRender.render() markdown\_highlight() zoomManager.apply("#cnblogs\_post\_body img:not(.code\_img\_closed):not(.code\_img\_opened)");
...
cb43a26c-3ba2-e611-845c-ac853d9f53ac

  自动补全

 [不改了](javascript:void\(0\);) [退出](javascript:void\(0\);) [订阅评论](javascript:void\(0\); "订阅后有新评论时会邮件通知您") [我的博客](https://www.cnblogs.com/my)
