...
| **Hook** | 扩展点 | 生命周期回调: before / after / on\_error / on\_timeout, 插件化插入横切逻辑 |
| **Mode** | 执行模式 | live(真实执行) / dry\_run(副作用拦截) / simulate(反事实模拟) |
...
| 安全与信任 | Guards + Mode | Guard 是闸门, dry\_run 是沙盒 |
...
![total](https://img2024.cnblogs.com/blog/1850883/202606/1850883-20260602211038651-1530811623.png)
...
![2 Tool Use](https://img2024.cnblogs.com/blog/1850883/202606/1850883-20260602211056300-171389067.png)
...
![3 ReAct](https://img2024.cnblogs.com/blog/1850883/202606/1850883-20260602211105310-507304675.png)
...
| 2\. State | S = |
| 3\. 拓扑 | 外层循环: Plan → Execute\_Step → Update → Plan(replan?) |
...
| 4\. Router | 基于 verification\_result 分支: pass → 下一步, fail → 重试或 replan |
...
| 2\. State | S 扩展为 S × M\_episodic × M\_semantic, memory 在"图外"可检索 |
...
| 4\. Router | 代码控制搜索策略 (BFS/DFS/beam/蒙特卡洛), LLM 只做 generate\_candidates + evaluate |
...
| 2\. State | S = |
| 3\. 拓扑 | fork → for each branch: rollout → evaluate → select\_best → execute(real) |
...
| Guard | **Mode 约束**: simulate 不修改 real\_env |
...
| 4\. Router | 基于 confidence 与 capability\_boundary 比较: confident → 执行; 否则 → 升级/拒绝 |
| 5\. 失败模式 | 过度自信 (self\_model 失真); 过度谨慎 (拒绝太多); 评估本身有成本 |
...
| 2\. State | S = S\_task × S\_memory × S\_gold (gold 为跨任务积累的最优解) |
...
| 坑 | 根因 | 解法 |
| --- | --- | --- |
| Agent 无限循环 | 缺少 max\_iterations 或 goal\_condition 永远不满足 | 硬上限 + 循环检测 (重复 action 计数) |
| Tool 调用幻觉 | LLM 生成不存在的 tool\_name 或参数 schema | strict mode function calling + schema 校验 |
...
[»](https://www.cnblogs.com/rossiXYZ/p/20296163) 下一篇： [Agent OS ：五种驯服不确定性的范式](https://www.cnblogs.com/rossiXYZ/p/20296163 "发布于 2026-06-04 20:46")

posted @ 2026-06-02 21:25  [罗西的思考](https://www.cnblogs.com/rossiXYZ)  阅读(435)  评论(0)    [收藏](javascript:void\(0\))  [举报](https://report.cnblogs.com?targetLink=https%3A%2F%2Fwww.cnblogs.com%2FrossiXYZ%2Fp%2F20274878&targetId=20274878&targetType=0)
